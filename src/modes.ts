/**
 * pi-planbuild v4 · 层 0：模式管理（工具可得性）+ 状态条 + 提示注入。
 *
...
/** 是否应在 plan 沙箱档把执行 shell 切到 powershell（v4 路线 X）。纯函数（可单测）。 */
...
export function registerModes(pi: ExtensionAPI, store: PlanbuildStore): ModeActions {
  ...
  function updateStatus(ctx: ExtensionContext): void {
    ...
    ctx.ui.setStatus("pi-planbuild", `${color}${badge}${STATUS_COLORS.reset}`);
  }
  ...
  function enterPlanMode(ctx: ExtensionContext, persist = true): void {
    ...
    // OS 沙箱档位联动：readonly/verify 需沙箱后端，不可用则降级 supervised
    ...
    // v4 路线 X：win32 + winacl 沙箱档 → 移除 bash、加入 powershell（沙箱 shell=pwsh）
    if (
  ...
  pi.on("before_agent_start", (event, ctx) => {
    ...
    // 模式切换 notice（dsh narration 语义）：模式变化时注入单句告知（幂等：notifiedMode 防抖）
